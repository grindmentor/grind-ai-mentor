...
import { Camera, Upload, Utensils, Zap } from "lucide-react";
...
const FoodPhotoLogger = ({ onFoodLogged }: FoodPhotoLoggerProps) => {
  const { user } = useAuth();
  ...
  const handleAnalyzeFood = async () => {
    ...
    try {
      const analysisPrompt = `Analyze this food photo and provide detailed nutritional information. Identify all visible foods and estimate their quantities. Provide:
      1. List of foods identified
      2. Estimated portion sizes
      3. Nutritional breakdown (calories, protein, carbs, fat, fiber)
      4. Total estimated values for the entire meal
      5. Any nutritional recommendations or observations
      
      Additional context: Meal type is ${mealType}. ${additionalNotes ? `Notes: ${additionalNotes}` : ''}
      
      Please be as accurate as possible with portion size estimates and provide scientific-based nutritional data.`;
  ...
  if (!canUseFeature('food_photo_analyses')) {
    return (
