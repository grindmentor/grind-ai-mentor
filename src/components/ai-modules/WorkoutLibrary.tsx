...
const WorkoutLibrary: React.FC<WorkoutLibraryProps> = ({ onBack }) => {
  ...
  const { exercises, loading } = useExerciseDatabase();

  // Use instant module loading
  const { isLoading: moduleLoading } = useInstantModule({
    moduleId: 'workout-library',
    preloadData: async () => exercises,
    minLoadTime: 200
  });

  // Preload exercises when tab changes
  useEffect(() => {
    if (activeTab === 'exercises' && exercises.length === 0) {
      // Exercises will be loaded by the hook
    }
  ...
  if (moduleLoading || loading) {
    return (
      <InstantLoader 
        isLoading={true} 
        message="Loading exercise database..." 
        variant="module" 
      />
    );
  }
            ...
            <div className="space-y-4 bg-gradient-to-r from-blue-900/20 to-blue-800/20 backdrop-blur-sm border border-blue-500/30 rounded-2xl p-4 sm:p-6">
              <div className="relative">
                ...
                <Input
                  ...
                  className="pl-12 bg-blue-800/30 border-blue-500/40 text-white placeholder:text-blue-300/70 h-14 text-lg focus:border-blue-400 rounded-2xl transition-all duration-200 touch-manipulation"
                />
              </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {filteredExercises.map((exercise, index) => (
                    <SmoothTransition 
                      ...
                      type="slideUp"
                    >
                      <Card className="bg-blue-900/40 border-blue-600/50 backdrop-blur-sm hover:bg-blue-900/60 transition-all duration-200 transform hover:scale-[1.02] active:scale-[0.98]">
          ...
          <div className="p-4 border-t border-gray-800">
            <div className="flex items-center space-x-2">
              <Input
                ...
                className="bg-gray-800/50 border-gray-700/50 text-white placeholder:text-gray-400 h-12 rounded-xl touch-manipulation"
              />
              <TouchButton
