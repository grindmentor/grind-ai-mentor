...
>(({ className, ...props }, ref) => {
  ...
  return (
    <TabsPrimitive.Trigger
      ref={ref}
      className={cn(
        "inline-flex items-center justify-center whitespace-nowrap rounded-sm px-3 py-1.5 text-sm font-medium ring-offset-background transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 data-[state=active]:bg-background data-[state=active]:text-foreground data-[state=active]:shadow-sm overflow-hidden text-ellipsis max-w-full",
        optimizedSettings.reduceAnimations ? "duration-100" : "duration-200",
        className
      )}
      {...props}
    />
...
>(({ className, children, ...props }, ref) => {
  ...
  React.useEffect(() => {
    const timer = setTimeout(() => {
      setIsVisible(true);
    }, optimizedSettings.reduceAnimations ? 50 : 100);
